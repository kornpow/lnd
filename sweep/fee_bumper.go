...
func (r *BumpRequest) MaxFeeRateAllowed() (chainfee.SatPerKWeight, error) {
	...
	// budget fee rate.
	size, err := calcSweepTxWeight(r.Inputs, r.ChangeScript)
	if err != nil {
		return 0, err
	}
...
func (t *TxPublisher) createRBFCompliantTx(req *BumpRequest,
	...
	for {
		...
		case errors.Is(err, lnwallet.ErrMempoolFee):
			log.Trace("Trying to increase fee for next round...")
...
func (t *TxPublisher) createAndCheckTx(req *BumpRequest, f FeeFunction) (
	*wire.MsgTx, btcutil.Amount, error) {
	...
	tx, fee, err := createSweepTx(
		req.Inputs, nil, req.ChangeScript, uint32(t.currentHeight),
