...
func TestBudgetInputSetClusterInputs(t *testing.T) {
	...
	for i, deadline := range []fn.Option[int32]{
		...
		// We expect the high budget input to call this method twice,
		// one in `filterInputs` and the other in `createInputSet`.
		inpHigh1.On("OutPoint").Return(&opHigh1).Twice()
		inpHigh2.On("OutPoint").Return(&opHigh2).Twice()
