...
func TestHandleBumpEventTxReplaced(t *testing.T) {
	...
	// Create a testing tx that spends the the input.
	tx := &wire.MsgTx{
		LockTime: 1,
		TxIn: []*wire.TxIn{
			{PreviousOutPoint: op},
...
// exits when the sweeper is stopped, an error is returned, or the tx is
// confirmed.
func TestMonitorFeeBumpResult(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// When a tx confirmed event is recieved, we expect to
			...
			// When an error is returned from handling the event,
			// we expect the monitor to exit.
			name: "exit on error",
			// We send a result with TxPublished and mock the
			// method `StoreTx` to return an error.
			setupResultChan: func() <-chan *BumpResult {
				// Create a result chan.
				resultChan := make(chan *BumpResult, 1)
				resultChan <- &BumpResult{
				...
				// Mock the store to return an error, causing
				// the loop to exit.
				store.On("StoreTx", &TxRecord{
					Txid:      tx.TxHash(),
					Published: true,
				}).Return(errors.New("dummy")).Once()
