...
func NewBudgetAggregator(estimator chainfee.Estimator,
	maxInputs int) *BudgetAggregator {
...
// cluster defines an alias for a set of inputs that are to be grouped.
type cluster map[fn.Option[int32]][]*pendingInput
...
func (b *BudgetAggregator) ClusterInputs(inputs pendingInputs) []InputSet {
	...
	clusters := make(cluster, len(filteredInputs))
...
func (b *BudgetAggregator) createInputSets(inputs []*pendingInput) []InputSet {
	...
	for len(remainingInputs) > b.maxInputs {
		...
		set := NewBudgetInputSet(inputs[:b.maxInputs])
	...
	if len(remainingInputs) > 0 {
		set := NewBudgetInputSet(remainingInputs)
		sets = append(sets, set)
	}
...
func (b *BudgetAggregator) sortInputs(inputs []*pendingInput) []*pendingInput {
	...
	// Copy the inputs.
	for _, pi := range inputs {
		sortedInputs = append(sortedInputs, pi)
	}
